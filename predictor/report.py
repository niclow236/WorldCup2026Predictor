"""
report.py — render the final evaluation: figures and ``final_report.md``.
...
def fig_title_timeline(timeline: pd.DataFrame, teams: list[str], finishes: dict, th: dict, path: str):
    ...
    for ax, team in zip(axes.ravel(), teams):
        ...
        ax.set_title(f"{team} — {finishes[team]}", loc="left", fontsize=10.5, color=th["ink"],
...
def _pct(p: float, d: int = 1) -> str:
    """0.184 -> '18.4%'."""
    return _fmt(100 * p, f".{d}f") + "%" if not np.isnan(p) else "—"
...
def write_final_report(res: dict, path: str) -> None:
    ...
    L += ["## 6 · The knockout bracket, projected vs actual", "",
          ...
          _md_table(pd.DataFrame({
              ...
              "Projected but missed": pr["projected_but_missed"].replace("", "—"),
              "Unexpected arrivals": pr["unexpected_arrivals"].replace("", "—"),
          })), "",
